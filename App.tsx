...
import { 
  RefreshCw, Trophy, BrainCircuit, X, Star, Sparkles, ShieldCheck, Volume2, AlertTriangle, Search, Filter, SlidersHorizontal, Radio, BarChart3, LineChart, FileSearch, History, Settings, Key, Info, ExternalLink, ClipboardList, ChevronRight, Clock, Bell, BellRing, Target, Flag, Lock
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const handleOpenApiKeyDialog = async () => {
    if (window.aistudio) {
       ...
       // Procediamo assumendo che la selezione sia andata a buon fine come da linee guida
       setHasApiKey(true);
       // Ricarichiamo i dati dopo la selezione
  ...
  const loadData = useCallback(async (isInitial = false, forceNoSearch = false, isSilent = false) => {
    ...
    try {
      ...
      // Gestione specifica dell'errore di chiave mancante o non trovata
      if (errorStr.includes("Requested entity was not found") || errorStr.includes("API key not valid")) {
        setHasApiKey(false);
        addNotification("Errore API Key", "La chiave selezionata non è valida o è stata revocata.", "info");
  ...
  const handlePredict = async (home: string, away: string) => {
    ...
    try {
      ...
      setPrediction({ prediction: "N/D", confidence: "0%", analysis: "Errore API o Chiave non valida." });
    } finally {
      setPredicting(false);
    }
  };
  ...
  const renderApiKeyOverlay = () => (
    <div className="fixed inset-0 z-[300] bg-emerald-950/90 backdrop-blur-xl flex items-center justify-center p-6 animate-in fade-in duration-500">
      <div className="max-w-md w-full bg-white rounded-[3.5rem] shadow-2xl p-12 text-center space-y-8 animate-in zoom-in-95">
        <div className="flex justify-center">
          <div className="relative">
            <div className="bg-emerald-50 p-6 rounded-full">
               <Lock className="w-16 h-16 text-emerald-600" />
            </div>
            <div className="absolute -top-2 -right-2 bg-lime-400 p-2 rounded-full border-4 border-white shadow-lg">
        ...
        <div className="space-y-3">
          <h2 className="text-3xl font-black text-slate-800 uppercase tracking-tighter">Accesso IA Richiesto</h2>
          <p className="text-sm text-slate-500 font-medium leading-relaxed px-4">
            Per sbloccare le funzionalità di analisi predittiva, statistiche avanzate e tracking in tempo reale, è necessaria una chiave API Gemini valida.
          </p>
        </div>
        ...
        <div className="space-y-4 pt-4">
          <button 
            ...
            className="w-full bg-emerald-800 text-white py-5 rounded-3xl font-black text-xs uppercase tracking-widest shadow-xl shadow-emerald-200 hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-3"
          >
            <Key className="w-5 h-5" /> Configura API Key
          </button>
          
          <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 space-y-2">
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Requisiti Chiave</p>
            <a 
              href="https://ai.google.dev/gemini-api/docs/billing" 
              target="_blank" 
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-2 text-[10px] font-black text-emerald-600 uppercase hover:underline transition-all"
            >
              <Info className="w-4 h-4" /> La chiave deve appartenere a un progetto con fatturazione attiva <ExternalLink className="w-3 h-3" />
            </a>
          </div>
        </div>
        
        <p className="text-[9px] text-slate-400 font-bold italic">La tua configurazione è salvata localmente nel browser.</p>
      </div>
    </div>
  );
        ...
        return (
          <div className="space-y-10 animate-in fade-in">
             ...
             {favorites.length > 0 ? (
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {favorites.map(fav => {
                    const activeNotifsCount = Object.values(fav.notifications).filter(Boolean).length;
                            ...
                            <div className="flex items-center gap-4">
                               ...
                               <div>
                                  ...
                                  <span className="text-[10px] font-black text-emerald-500 uppercase flex items-center gap-1.5">
                                     <BellRing className={`w-3 h-3 ${activeNotifsCount > 0 ? 'animate-pulse text-emerald-600' : 'text-slate-300'}`} /> 
                                     Alert Attivi
                                     {activeNotifsCount > 0 && (
                                       <span className={`ml-1.5 px-2 py-0.5 rounded-full text-[8px] font-black tracking-tighter border shadow-sm transition-colors duration-300 ${
                                         activeNotifsCount === 3 
                                           ? 'bg-lime-400 border-lime-500 text-emerald-900 shadow-lime-200' 
                                           : 'bg-amber-100 border-amber-200 text-amber-700'
                                       }`}>
                                         {activeNotifsCount === 3 ? 'FULL' : 'ATTIVI'}
                                       </span>
                                     )}
                                  </span>
                               </div>
